...
import { ArrowDown, ArrowUp, Pencil, Pin, Plus, Search, Trash2, X } from "lucide-react"
...
export function MarketsWorkspace({ accounts, initialWatchlists, initialInstrument, initialNews, initialCalendar }: {
  ...
  function openEdit() { if (!watchlist) return; setEditName(watchlist.name); setEditError(""); setEditOpen(true) }
  async function renameWatchlist() { if (!watchlist || !editName.trim()) return; try { const updated = await mutateAfd<Watchlist>(`watchlists/${watchlist.id}`, "PUT", { name: editName.trim(), items: watchlist.items }); setWatchlists((current) => current.map((item) => item.id === updated.id ? updated : item)); setEditOpen(false) } catch (error) { setEditError(error instanceof Error ? error.message : "Unable to rename this watchlist.") } }
  async function deleteWatchlist() { if (!watchlist || watchlist.is_default) return; try { await mutateAfd(`watchlists/${watchlist.id}`, "DELETE"); const remaining = watchlists.filter((item) => item.id !== watchlist.id); setWatchlists(remaining); setWatchlistId(remaining[0]?.id ?? ""); setEditOpen(false) } catch (error) { setEditError(error instanceof Error ? error.message : "Unable to delete this watchlist.") } }
  async function removeItem(symbol: string) { if (!watchlist) return; await saveItems(watchlist.items.filter((item) => item.symbol !== symbol).map((item, position) => ({ ...item, position }))) }
  ...
  return <div className="space-y-5"><div className="grid gap-5 xl:grid-cols-[.75fr_2fr]">
    <Card><CardHeader><div className="flex items-center justify-between"><CardTitle>Watchlists</CardTitle><div className="flex gap-1"><Button size="icon" variant="ghost" onClick={openEdit} disabled={!watchlist} aria-label="Edit watchlist"><Pencil /></Button><Button size="icon" variant="ghost" onClick={() => setCreateOpen(true)} aria-label="Create watchlist"><Plus /></Button></div></div></CardHeader><CardContent>
...
function EconomicCalendar({ data, error, filter, setFilter, expanded, setExpanded }: { data:CalendarData; error?:string; filter:"relevant"|"high"|"all"; setFilter:(value:"relevant"|"high"|"all")=>void; expanded:string|null; setExpanded:(value:string|null)=>void }) {
  ...
  return <Card><CardHeader><CardTitle>Economic Calendar</CardTitle></CardHeader><CardContent><div className="mb-3 flex flex-wrap gap-1">{([["relevant","Relevant to Selected Market"],["high","High Impact"],["all","All Events"]] as const).map(([value,label]) => <Button key={value} size="sm" variant={filter === value ? "default" : "outline"} onClick={() => setFilter(value)}>{label}</Button>)}</div>{error ? <SectionMessage message={error} /> : <div className="space-y-3">{items.slice(0, 8).map((item, index) => { const key=`${item.title}-${item.scheduled_time}-${index}`; return <div key={key} className="rounded border p-3 text-sm"><button className="w-full text-left" onClick={() => setExpanded(expanded === key ? null : key)} aria-expanded={expanded === key}><strong>{item.title}</strong><p className="text-xs text-muted-foreground">{item.currency ?? "Global"} · {formatEnum(item.impact)}{item.scheduled_time ? ` · ${new Date(item.scheduled_time).toLocaleString()}` : ""}</p>{item.relevance_label && <p className="mt-1 text-xs text-emerald-600">{item.relevance_label}</p>}</button><Source label={item.source}/>{expanded === key && item.associated_fred_series.map((series) => <div key={series.series_id} className="mt-2 rounded bg-muted p-2 text-xs"><strong>{series.label}</strong><p>Latest: {series.value ?? "Unavailable"} {series.unit ?? ""} · Previous: {series.previous ?? "Unavailable"}</p><p>Recent trend: {series.trend.join(" → ") || "Unavailable"}</p><Source label={series.source} timestamp={series.observation_date} status={series.stale?"Stale":"Current"}/></div>)}</div>})}{!items.length && <SectionMessage message="No events match this filter." />}</div>}<Source label={data.sources?.join(", ") || "finnhub / fred"} timestamp={data.last_updated} /></CardContent></Card>
}
...
function Source({ label, timestamp, status }: { label: string; timestamp?: string; status?: string }) { const providers=normalizeSourceProviders(label);return <p className="mt-3 text-xs text-muted-foreground">Source: {providers}{status ? ` · ${formatEnum(status)}` : ""}{timestamp ? ` · Updated ${sourceTimestamp(timestamp)}` : ""}</p> }
function sourceTimestamp(value:string){if(/^\d{4}-\d{2}-\d{2}$/.test(value))return value;const parsed=new Date(value);return Number.isNaN(parsed.getTime())?value:parsed.toLocaleString()}
