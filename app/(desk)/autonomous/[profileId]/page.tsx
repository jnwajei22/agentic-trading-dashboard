...
export default async function ProfilePage({ params }: { params: Promise<{ profileId: string }> }) {
  ...
  return <div className="space-y-7"><PageHeader eyebrow="Execution Profile V2" title={`Profile · ${contract.account_alias}`} description="Backend defaults, validation ranges, market groups, capabilities, warnings, and migration state drive this editor." /><ProfileEditor contract={contract} /></div>
}
